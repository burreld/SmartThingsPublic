...
def get_APP_VERSION() {return "7.5.1"}
...
def initialize() {
	...
	for (int i = 1;
		...
		if (motionSensor) {
			...
			subscribe(motionSensor, "motion",{evt -> motionEvtHandler(evt, i)  }, [filterEvents: false])
		}    
		key ="contactSensor${i}"
		def contactSensor = settings[key]
		...
		if (contactSensor) {
			...
			subscribe(contactSensor, "contact.closed", { evt -> contactEvtHandler(evt, i) }, [filterEvents: false])
			subscribe(contactSensor, "contact.open", { evt -> contactEvtHandler(evt, i) }, [filterEvents: false])
		}            
